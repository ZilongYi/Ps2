...
fpc.srs = append(fpc.srs, c(rep(119650223,size[2]), rep(3493200,size[3]),rep(821282,size[4]),
                            rep(939283,size[5]),rep(648745,size[6]),
                            rep(1087883,size[7]), rep(453724,size[8]),
                            rep(4118671,size[9]),rep(127698,size[10])))
birth.design <- svydesign(id=~1, data=data, fpc = fpc.srs)
...
```
By given table, each coefficient, i.e. $\beta_i$, has been calculeted or modeled. Thus aiming model would be 
$$\hat{y_i} = 30.417 + (-0.038)*x_{1i}* + x_{2i}*\beta_2 + x_{3i}*\beta_3 + + x_{4i}*\beta_4+ x_{5i} *\beta_5 $$
Note variable $x_{i}\ for\  i=2,3,4,5 $ are categorical variables, it is expected to have different coeffcients for different levels. Taking income_family as an example, for level of 125,000 and more, the coeffcient is 1.19813569.
